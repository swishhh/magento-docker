...
RUN wget -O basic.rpm https://download.oracle.com/otn_software/linux/instantclient/instantclient-basiclite-linux-arm64.rpm && \
    ...
    export LD_LIBRARY_PATH=/usr/lib/oracle/19.22/client64/lib/${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH} && \
    touch /etc/ld.so.conf.d/oracle.conf && \
    echo -e '/usr/lib/oracle/19.22/client64/lib/' >> /etc/ld.so.conf.d/oracle.conf  && \
    ldconfig && \
    touch /etc/profile.d/oracle.sh && chmod o+r /etc/profile.d/oracle.sh && \
    echo -e 'export ORACLE_HOME=/usr/lib/oracle/19.22/client64 \n export PATH=$PATH:$ORACLE_HOME/bin' >> /etc/profile.d/oracle.sh && \
    echo -e '\n export ORACLE_HOME=/usr/lib/oracle/19.22/client64' >> ~/.bash_profile && \
    ln -s /usr/include/oracle/19.22/client64 $ORACLE_HOME/include

#instantclient,/usr/lib/oracle/19.22/client64/lib
RUN pecl install -f oci8 && \
    touch /etc/php/8.2/mods-available/oci8.ini && \
    echo "extension=oci8.so" > /etc/php/8.2/mods-available/oci8.ini \
    && ln -s /etc/php/8.2/mods-available/oci8.ini /etc/php/8.2/cli/conf.d/666-oci8.ini \
    && ln -s /etc/php/8.2/mods-available/oci8.ini /etc/php/8.2/fpm/conf.d/666-oci8.ini

RUN pecl install -f yaml && \
    touch /etc/php/8.2/mods-available/yaml.ini && \
    echo "extension=yaml.so" > /etc/php/8.2/mods-available/yaml.ini \
    && ln -s /etc/php/8.2/mods-available/yaml.ini /etc/php/8.2/cli/conf.d/777-yaml.ini \
    && ln -s /etc/php/8.2/mods-available/yaml.ini /etc/php/8.2/fpm/conf.d/777-yaml.ini
